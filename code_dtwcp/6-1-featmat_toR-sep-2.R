...
dataPath <- '~/Documents/Data/Project2/Ripoll2014/REVISION-RIPOLL/featmats-default-sep/'
...
fmList_R_10splits_default$day1$split10$comp28 %>% head

# ------ normalized ------ # 

# try <- getUnitComp(fmList_R_10splits_default$day1$split8$comp25)

fmList_norm <- list()
for(d in 1:7){
  fmList_norm_perday <- list()
  for(s in 1:10){
    fmList_norm_perday[[s]] <- map(fmList_R_10splits_default[[d]][[s]], getUnitComp)
    
  }
  names(fmList_norm_perday) <- paste0('split', 1:10)
  fmList_norm[[d]] <- fmList_norm_perday
}
names(fmList_norm) <- paste0('day', 1:7)


# x <- fmList_norm$day7$split10$comp28[, 1]
# sum(x^2)

saveRDS(fmList_norm, file = paste0(dataPath, 'featmat-sep-norm.RData'))
